...
@section('content')
    <div class="sidebodydiv px-5 py-3">
        <div class="sidebodyhead">
            <h4 class="m-0">Attendance Report</h4>
        </div>

        <form method="post" id="form" action="{{ route('attendance.ind_report') }}">
            @csrf

            <div class="container-fluid maindiv my-3">
                {{-- <div class="row"> --}}
                {{-- <div class="col-sm-12 col-md-4 col-xl-4 inputs mb-3">
                        <label for="dept">Date <span>*</span></label>
                ...
                {{-- <div class="col-sm-12 col-md-4 col-xl-4 inputs mb-3">
                        ...
                        <select class="form-select" name="dept" id="dept" autofocus required>
                            ...
                            @foreach ($dept as $item)
                                <option value="{{ $item->role_dept }}"
                                    {{ old('dept', $selected_dept ?? '') == $item->role_dept ? 'selected' : '' }}>
                                    {{ $item->role_dept }}
                                </option>
                            @endforeach
                        </select>
                    </div> --}}
                {{-- <div class="col-sm-12 col-md-4 col-xl-4 inputs mb-3" id="store_div"
                        style="display:{{ old('dept', $selected_dept ?? '') == 'Store' ? 'block' : 'none' }}">
                        <label for="stores">Stores <span>*</span></label>
                        <select class="form-select" name="stores" id="stores"
                            {{ old('dept', $selected_dept ?? '') == 'Store' ? 'required' : '' }}>
                            <option value="" selected disabled>Select Options</option>
                            @foreach ($stores as $store)
                                <option value="{{ $store->id }}"
                                    {{ old('stores', $selected_store ?? '') == $store->id ? 'selected' : '' }}>
                                    {{ $store->store_name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-sm-12 col-md-4 mb-3">
                        ...
                        <select class="form-select" name="employee_id" id="employee_id">
                            ...
                            @foreach ($employees as $emp)
                                <option value="{{ $emp->id }}" {{ old('employee_id', $selected_employee ?? '') == $emp->id ? 'selected' : '' }}>
                                    {{ $emp->name }}
                                </option>
                            @endforeach
                        </select>
        ...
        <div class="container-fluid listtable mt-4">
            ...
            <div class="table-wrapper">
                <table id="example1" class="table-hover table-striped table">
                    ...
                    <tbody>
                        {{-- @foreach ($emp_data ?? [] as $entry)
                            @php
                                if ($entry->attendance->isNotEmpty()) {
                                    $status = 'Present';
                                } elseif ($entry->leaves->isNotEmpty()) {
                                    $status = 'On Leave';
                                } else {
                                    $status = 'Absent';
                                }
                            ...
                            <tr>
                                ...
                                <td>{{ $status }}</td>
                                <td>{{ $entry->leaves->first()?->request_type ?? '' }}</td>
                                <td>{{ $entry->attendance->first()?->in_location ?? '' }}</td>
                                <td>{{ $entry->attendance->first()?->in_time ?? '' }}</td>
                                <td>{{ $entry->attendance->first()?->out_location ?? '' }}</td>
                                <td>{{ $entry->attendance->first()?->out_time ?? '' }}</td>
                            </tr>
                        @endforeach --}}
                        @foreach ($emp_data as $empDays)
                            @foreach ($empDays as $entry)
                                <tr>
                                    <td>{{ $loop->iteration }}</td>
                                    <td>{{ $entry['name'] }}</td>
                                    <td>{{ $entry['date'] }}</td>
    ...
    <script>
        ...
        //                 $('#employee').append('<option value="' + value.id + '" ' + selected + '>' +
        //                     value.name + '</option>');
        //             });
        //         },
