...
<div class="sidebodyhead mt-3">
    <h4 class="m-0">Purchase Order Approval List</h4>
</div>
            ...
            <tbody>
                @foreach ($pur_list as $wl)
                    <tr>
                        ...
                        <td>
                            @if ($wl->pur_file)
                                <a href="{{ asset('assets/images/Task/' . $wl->pur_file) }}" download>{{ $wl->pur_file }}</a>
                            @else
                                No File
                            @endif
                        </td>
                        ...
                        <td>
                            @if ($wl->status == 'Pending')
                                <button class="listtdbtn" data-bs-toggle="modal" data-id="{{ $wl->id }}" data-bs-target="#updateMaintenanceApproval">Update</button>
                            @elseif($wl->escalate_status == 'Escalate')
                                <button class="listtdbtn2" data-bs-toggle="modal" data-id="{{ $wl->id }}" data-bs-target="#updateApproval">Update</button>
                            @else
                                {{ $wl->escalate_status }}
                            @endif
                        </td>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('storepurchase_order') }}" method="POST" id="">
                    @csrf
                    <input type="hidden" id="rep_id" name="rep_id">
                    <div class="col-sm-12 col-md-12 mb-3">
                        <label for="sts" class="col-form-label">Status</label>
                        <select class="form-select sts" name="pur_status" id="sts" required>
                            <option value="" selected disabled>Select Options</option>
                            @if (auth()->user()->role_id == 41)
...
<div class="modal fade" id="updateApproval" tabindex="-1" aria-labelledby="updateApprovalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('storepurchaseapprove_order') }}" method="POST" id="">
                    @csrf
                    <input type="hidden" id="rep_id2" name="apo_id">
                    <div class="col-sm-12 col-md-12 mb-3">
                        <label for="sts" class="col-form-label">Status</label>
                        <select class="form-select sts" name="apo_status" id="sts" required>
                            <option value="" selected disabled>Select Options</option>
                            <option value="Approved">Approve</option>
                            <option value="Rejected">Rejected</option>
                        </select>
...
<script>
    ...
    $(document).on('click', '.listtdbtn2', function() {
        var apo_id = $(this).data('id');
        $('#rep_id2').val(apo_id);
    });
</script>
