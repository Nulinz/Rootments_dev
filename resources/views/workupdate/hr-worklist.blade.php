    ...
    @section('content')
        ...
        <div class="sidebodydiv px-5 py-3">
            <div class="sidebodyhead">
                <h4 class="m-0">HR Work Update</h4>

                @if (!in_array(auth()->user()->role_id, [1, 2]))
                    <a href="{{ route('hr_addwork') }}"><button class="listbtn">Add update</button></a>
                @endif
            </div>
            ...
            <div class="container-fluid listtable mt-4">
                ...
                <div class="table-wrapper">
                    <table id="example" class="table-hover table-striped table">
                        ...
                        <tbody>
                            @foreach ($hr_work as $wl)
                                <tr>
                                    <td>{{ $loop->iteration }}</td>
                                    <td>{{ $wl->attrition_rate }}</td>
                                    <td>{{ $wl->retention_rate }}</td>
                                    <td>{{ $wl->hiring_completion }}</td>
